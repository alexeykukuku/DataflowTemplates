...
/**
 * Class {@link FailsafePublisher} provides methods that generates pubsub
 * message and publishes it to PubSub topic. The data failed to publish is returned as the output.
 */
public final class FailsafePublisher {
  ...
  public static class PublishModJsonToTopic
      extends PTransform<PCollection<FailsafeElement<String, String>>, PCollection<FailsafeElement<String, String>>> {
    ...
    public PCollection<FailsafeElement<String, String>> expand(PCollection<FailsafeElement<String, String>> input) {
      return input.apply(
              ParDo.of(new PublishModJsonToTopicFn(pubSubUtils)))
              .setCoder(failsafeModJsonToPubsubMessageOptions.getCoder());
    }

    /**
     * The {@link PublishModJsonToTopicFn} converts a JSON string wrapped in {@link
     * FailsafeElement} to a {@link PubsubMessage} and publishes it to the topic.
     */
    public static class PublishModJsonToTopicFn
      ...
      public void processElement(ProcessContext context) {
        ...
        try {
          ...
          LOG.info("MSG ID: {}", publisher.publish(pubSubMessage).get());
        } catch (Exception e) {
          LOG.warn("Failed to publish the message", e);
          context.output(FailsafeElement.of(failsafeModJsonString)
      ...
      /* Schema Details:  */
      private PubsubMessage newPubsubMessage(String modJsonString)
          throws Exception {
          ...
          default:
            final String errorMessage =
                "Invalid message format:"
                    + messageFormat
                    + ". Supported output formats: " + Arrays.toString(MessageFormat.values());
  ...
  public abstract static class FailsafeModJsonToPubsubMessageOptions implements Serializable {
    ...
    static Builder builder() {
      return new AutoValue_FailsafePublisher_FailsafeModJsonToPubsubMessageOptions
          .Builder();
    }
