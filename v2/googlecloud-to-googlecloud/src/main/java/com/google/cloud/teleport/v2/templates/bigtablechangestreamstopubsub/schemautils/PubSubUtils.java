...
public class PubSubUtils implements Serializable {
  ...
  static {
    ...
    FORMATTERS.put(
        ...
        (pb, chg) -> chg.getString(PubSubFields.VALUE_STRING_BASE64.name()));
    FORMATTERS.put(
  ...
  public PubsubMessage mapChangeJsonStringToPubSubMessageAsJson(String changeJsonSting)
    ...
    Long timestampFrom = (Long) FORMATTERS.get(PubSubFields.TIMESTAMP_FROM)
        .format(this, changeJsonParsed);
    if (timestampFrom != null) {
      changelogEntryTextBuilder.setTimestampFrom(timestampFrom);
    }
    Long timestampTo = (Long) FORMATTERS.get(PubSubFields.TIMESTAMP_TO)
        .format(this, changeJsonParsed);
    if (timestampTo != null) {
      changelogEntryTextBuilder.setTimestampTo(timestampTo);
    }

    ChangelogEntryMessageText.ChangelogEntryText changelogEntryMessageText = changelogEntryTextBuilder.build();
    PubsubMessage.Builder message = PubsubMessage.newBuilder();

    String jsonString =
        JsonFormat.printer()
            .omittingInsignificantWhitespace()
            .print(changelogEntryMessageText);
  ...
  public PubsubMessage mapChangeJsonStringToPubSubMessageAsProto(String changeJsonSting)
    ...
    byte[] columnBytes = (byte[]) FORMATTERS.get(PubSubFields.COLUMN_BYTES)
        .format(this, changeJsonParsed);
    if (columnBytes != null) {
    ...
    if (!destination.getStripValues()) {
      byte[] valueBytes = (byte[]) FORMATTERS.get(PubSubFields.VALUE_BYTES)
          .format(this, changeJsonParsed);
      if (valueBytes != null) {
    ...
    ChangelogEntryMessageProto.ChangelogEntryProto changelogEntryMessageProto = changelogEntryProtoBuilder.build();
    PubsubMessage.Builder message = PubsubMessage.newBuilder();
