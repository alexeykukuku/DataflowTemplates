 ...
 * {@link ChangeStreamMutation} is then broken into {@link Mod}, which converted into PubsubMessage
 * and inserted into Pub/Sub topic.
 */
@Template(
...
public final class BigtableChangeStreamsToPubSub {
  ...
  public static PipelineResult run(BigtableChangeStreamsToPubSubOptions options) {
    ...
    FailsafePublisher.FailsafeModJsonToPubsubMessageOptions
  ...
  private static Boolean validateSchema(
      Topic topic,
      BigtableChangeStreamsToPubSubOptions options,
      BigtableSource source) throws Exception {
    ...
    if (topic.getSchemaSettings().getSchema().isEmpty()) {
      ...
      LOG.info("Pipeline will use message format: {}, message encoding: {}",
          options.getMessageFormat(),
          options.getMessageEncoding());
  ...
  private static PubsubMessage createTestChangeMessage(PubSubUtils pubSub) throws Exception {
    SetCell setCell = SetCell.create(
        "test_column_family",
    ...
    TestChangeStreamMutation mutation = new TestChangeStreamMutation(
        "test_rowkey",
        MutationType.USER,
        "source_cluster",
