...
public class DummyLogger extends PTransform<PCollection<ChangeStreamMutation>, PCollection<ChangeStreamMutation>> {
  ...
  @Override
  public PCollection<ChangeStreamMutation> expand(PCollection<ChangeStreamMutation> input) {
    return input.apply("Just log periodically", ParDo.of(new LogDoFn<>()));
  }
  ...
  private static class LogDoFn<T> extends DoFn<T, T> {
    ...
    public void processElement(@Element T element, OutputReceiver<T> r) {
      ...
      if (LAST_LOGGED.get() < now - 60000) {
        LOG.info("Worker {} processed {} records since the last time", WORKER_ID, RECORDS_PROCESSED.getAndSet(0));
        LAST_LOGGED.set(now);
      }
    }
  }
